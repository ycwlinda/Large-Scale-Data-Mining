...
depth_value=range(1,21)
for depth in depth_value:
    this_RMSE = list()
    OutOfBagError=list()
    for num_feature in range(1,6):
        rf = RandomForestRegressor(n_estimators = 12, max_depth = depth, max_features=num_feature, bootstrap=True ,oob_score=True)
        rf.fit(Feature,Result)
        predicted_target = cross_val_predict(rf, Feature, Result, cv = 10)
        print 'working on depth_value = '
        print depth
        ...
        OOBerror[num_feature-1][depth-1] = OutOfBagError[num_feature-1] 
    
        RMSE[num_feature-1][depth-1] = this_RMSE[num_feature-1] 
        
color_list = ['b', 'g', 'r', 'c', 'm']
    
plt.figure(figsize=(10,6))
for i in range(0, 5):
    plt.plot(depth_value, RMSE[i], c = color_list[i],  label = 'Feature '+str(i+1))
    
plt.xlabel('depth_value', fontsize = 20)
plt.ylabel('Test RMSE ', fontsize = 20)
plt.title('Test RMSE VS num of depth ', fontsize = 20)
...
plt.figure(figsize = (10,6))
for i in range(0, 5):
    plt.plot(depth_value, OOBerror[i], c = color_list[i],  label = 'Feature '+str(i+1))

plt.xlabel('depth_value', fontsize = 20)
plt.ylabel('OOBerror', fontsize = 20)
plt.title('OOBerror VS num of depth ', fontsize = 20)
plt.legend(loc="upper right")
plt.show()
